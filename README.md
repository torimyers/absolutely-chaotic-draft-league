...
- **Backend** (all optional): Cloudflare Pages Functions + D1. Three databases - cross-device league settings, Sleeper's player list refreshed daily, and the NFL schedule refreshed weekly, the last two by Workers on Cron Triggers
- **PWA**: Installable with offline support
- **Caching**: Three layers for the player database - the D1 table, IndexedDB in the browser, and an in-memory map per page
- **No hardcoded season data**: bye weeks are derived from the cached schedule rather than maintained by hand each August
